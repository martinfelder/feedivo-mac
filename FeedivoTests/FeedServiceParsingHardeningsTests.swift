...
struct FeedServiceParsingHardeningsTests {
    ...
    // --- Link-Trimming-Konsistenz (Task 5 Fix) ---

    @Test func parseRSSFeedTrimmtWhitespaceOnlyLinkZuNilUndErzeugtSynthID() throws {
        let xml = """
        <?xml version="1.0"?>
        <rss version="2.0">
          <channel><title>Test</title>
            <item>
              <title>Artikel mit Whitespace-Link</title>
              <link>   </link>
              <pubDate>Wed, 01 Jul 2026 10:00:00 GMT</pubDate>
            </item>
          </channel>
        ...
        #expect(feed.articles.first?.link == nil)
        #expect(feed.articles.first?.sourceID?.hasPrefix("synth:") == true)
    }
}