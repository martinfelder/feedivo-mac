...
1. **Gezielte statt globale Reload-Invalidierung.** `stickyRowSnapshots` mildert nur das Symptom (Verschwinden), löst aber nicht das Grundproblem: `bumpStatusVersion()` ist ein einziger globaler Zähler für alle Mutationsarten. Feinere Granularität (getrennte Versionszähler pro Scope-Kategorie, oder ein direktes "diese ID hat sich geändert"-Signal) würde den Rebuild-Rhythmus stark reduzieren — analog zu NNWs `representSameArticlesInSameOrder`-Check vor dem Reload.
   ...
   **Live-Verifikation (2026-07-28) via computer-use, beide Teilfixe (b) und (c) bestätigt.** (c) Sticky-Verhalten für Stern/Archiv: in „Mit Stern" einen Artikel entsternt — Sidebar-Badge fiel sofort von 7 auf 6, der Artikel blieb aber sichtbar (Stern-Icon leer) bis zum Ordnerwechsel, danach korrekt verschwunden (6 verbleibende). Symmetrisch für Archivieren getestet: Artikel aus „Alle Artikel" archiviert → erschien korrekt in „Archiviert"; dort wieder „Aus Archiv entfernt" → Badge verschwand sofort, Artikel blieb sticky sichtbar, nach Ordnerwechsel weg/zurück korrekt leer. Beide Male exakt das beabsichtigte Verhalten, keine Regression. (b) Debounce: in „Ungelesen" (10 Artikel) 5 Artikel in schneller Folge als gelesen markiert (Klick auf den Ungelesen-Punkt, <1s Abstand) — Badge aktualisierte korrekt von 10 auf 5, alle 10 Zeilen blieben sticky sichtbar (gelesene grau/nicht-fett dargestellt), kein Absturz, keine Dateninkonsistenz; nach Ordnerwechsel weg/zurück korrekt nur noch 5 tatsächlich ungelesene Zeilen. Bündelung mehrerer schneller `bumpStatusVersion()`-Aufrufe funktioniert im Live-Betrieb fehlerfrei.
2. **GRDB-Statement-Caching prüfen/erzwingen** als Äquivalent zu `setShouldCacheStatements(true)`, insbesondere für die Hot-Path-Queries (Timeline, Sidebar-CTE).
   **Umgesetzt (2026-07-28):** Im lokalen GRDB-Quellcode verifiziert (nicht geraten): `FetchableRecord.fetchAll(db, sql:...)`/`Row.fetchAll(db, sql:...)` routen über `SQLRequest(..., cached: Bool = false)` — Default `false`, von den SQL-String-Convenience-Overloads nicht mal exponiert. `grep` über den ganzen Baum: **null** Stellen mit `cached: true`/`cachedStatement` — jede Query in Feedivo wurde bislang bei jedem Aufruf frisch geparst/kompiliert. `TimelineStore.articles(...)`/`.articlesAsync(...)` und `FeedStore.querySidebarFeeds(_:)` auf `SQLRequest<T>(sql:..., cached: true).fetchAll(db)` umgestellt (restliche, seltener aufgerufene SQL-Stellen bewusst unangetastet). Dabei einen Compile-Fallstrick gefunden: `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` machte `ArticleListSnapshot`s/`FeedSidebarSnapshot`s `FetchableRecord`-Konformität implizit MainActor-isoliert, was `SQLRequest<T>`s Sendable-Anforderung verletzte — behoben durch `nonisolated` auf beiden `init(row:)`. Design-Doc: `docs/superpowers/specs/2026-07-28-grdb-statement-caching-design.md`.
   **Live-Verifikation (2026-07-28) via computer-use.** Da Statement-Caching primär ein Korrektheitsrisiko birgt (ein wiederverwendetes `SQLRequest`/`sqlite3_stmt` mit falsch neu gebundenen Parametern könnte Ergebnisse aus einem vorherigen Aufruf durchmischen), gezielt auf genau dieses Risiko getestet statt nur auf Performance: 6 verschiedene Sidebar-Selektionen nacheinander angeklickt (3 Einzel-Feeds mit unterschiedlichem `feedID`-Bind-Parameter — Talk Android, SecurityWeek, The Hacker News; Smart Folder „Mit Stern" mit 7 Artikeln aus 5 verschiedenen Feeds; zusätzlich 4× schnelles Hin-und-Herwechseln zwischen zwei Feeds direkt hintereinander, um Statement-Wiederverwendung mit wechselnden Bind-Parametern gezielt zu stressen). Jede Selektion zeigte durchgehend exakt die korrekten, zum jeweiligen Feed/Ordner passenden Artikel — keine Cross-Contamination zwischen aufeinanderfolgenden Aufrufen mit unterschiedlichen Parametern, keine Abstürze, keine sichtbaren Verzögerungen beim Umschalten. Live bestätigt: die `cached: true`-Umstellung auf `TimelineStore`/`FeedStore` funktioniert korrekt im laufenden Betrieb.
3. **Content-Hash-Vergleich nach 200 OK** ergänzen/verifizieren — Feedivo hat laut CLAUDE.md bereits `lastBodyHash` als Feld; prüfen, ob es beim Refresh tatsächlich zum Skip-Parse genutzt wird oder nur gespeichert.
   **Verifiziert, bereits vollständig umgesetzt (2026-07-28), kein Codefix nötig:** Kompletter Pfad Ende-zu-Ende gelesen und per gezieltem Testlauf bestätigt (12/12 grün). `FeedService.fetchFeedConditionally` (`FeedService.swift:245-252`) vergleicht bei einer 200-OK-Antwort ohne 304 den SHA256-Hash des neuen Bodys (`FeedService.contentHash(for:)`, Zeile 637-640) gegen den übergebenen `validators.contentHash` und liefert bei Gleichheit `.notModified` zurück — **ohne** `parseFeed(...)` überhaupt aufzurufen (echtes frühes Return vor dem Parsing-Aufruf, kein Skip-Parse-Fake). Explizit durch `conditionalFetchGibtNotModifiedBeiUnveraendertemBodyHashZurueck` (`FeedivoTests/FeedServiceConditionalFetchTests.swift:81-112`) abgesichert — der Test übergibt bewusst nicht-parsbaren Body (`"kein feed xml"`) mit passendem Hash und prüft, dass `.notModified` zurückkommt statt eines Parse-Fehlers. `SQLiteFeedRefreshService.refresh(feedID:)` (`SQLiteFeedRefreshService.swift:96-121`) behandelt den `.notModified`-Fall bereits vollständig: kein `articleStore.upsert`, keine `applyRules`, keine Spotlight-Indexierung (separat abgesichert durch `refreshRuftSpotlightIndexierungNichtBeiNotModifiedAuf`) — nur Validators (inkl. `lastBodyHash`) + Log-Eintrag „Nicht geändert" werden geschrieben. `FeedStore.updateAfterRefresh` (`FeedStore.swift:218-260`) persistiert `validators.contentHash` zuverlässig zurück in `feeds.lastBodyHash` für den nächsten Refresh-Zyklus, in beiden Zweigen (`.updated` UND `.notModified`). Diese Infrastruktur (ETag/If-Modified-Since **und** Content-Hash-Fallback) existierte bereits vor dem NetNewsWire-Vergleich vom 2026-07-27 — der offene Punkt war rein eine Verifikationsfrage, keine fehlende Funktionalität.
...
4. **Feste/vorab berechnete Zeilenhöhe statt Auto-Layout pro Zeile** in `ArticleRowView`, falls die Zeilenhöhe ohnehin meist uniform sein kann. Vermutlich größter Einzelhebel (NNWs Faktor 1), aber erst prüfen, ob Feedivos variable Thumbnail-Größen/2-3-zeilige Summaries (Feature 19.1) eine feste Höhe optisch zulassen.
   **Umgesetzt (2026-07-28):** Brainstorming ergab Nutzer-Zustimmung zum Leerraum-Trade-off (kurze Titel/Summaries schmiegen sich nicht mehr eng an, sondern lassen Platz bis zur festen Höhe). Neue, reine Funktion `ArticleRowHeightMetrics.height(interfaceTextSize:imagePosition:summaryLineCount:)` (`Feedivo/Views/ArticleList/ArticleRowHeightMetrics.swift`) berechnet die Höhe ausschließlich aus den drei app-weiten Anzeige-Einstellungen — nie aus Artikelinhalt — über `NSLayoutManager().defaultLineHeight(for:)` (Titel immer 2 Zeilen, Metadaten-Zeile immer 1 Zeile reserviert, Summary 0–3 Zeilen je Einstellung) plus Bildhöhe als Untergrenze, VStack-Spacing, Padding und 4pt-Sicherheitspuffer gegen `NSFont`/SwiftUI-`Text`-Metrik-Abweichungen. `ArticleRowView` wendet das Ergebnis über `.frame(height:, alignment: .top)` auf die äußere `HStack` an; die rechte Spalte (Stern-Button) nutzt dieselbe (Inhalts-)Höhe statt der bisherigen hartcodierten, nicht textgrößen-skalierten `76`. TDD (4 neue Tests, `ArticleRowHeightMetricsTests.swift`), Build grün. Design-Doc: `docs/superpowers/specs/2026-07-28-artikelliste-feste-zeilenhoehe-design.md`. Ansatz B (gemessener Off-Screen-Prototyp wie NetNewsWire) wurde diskutiert und zugunsten der reinen, testbaren Funktion verworfen (Risiko: `NSHostingView.fittingSize`-Fallstricke).
   **Live-Verifikation (2026-07-28) via computer-use auf einem frisch gebauten Debug-Build durchgeführt.** Minimalfall (Klein/kein Bild/0 Summary-Zeilen) und Standardfall (Groß/Bild links/2 Zeilen) live geprüft: gleichmäßige Zeilenhöhe, Stern-Button nie abgeschnitten, korrektes Leerraum-Verhalten bei kurzen Titeln/Summaries wie geplant. **Maximalfall (Sehr groß/Bild links/3 Zeilen) zeigte einmalig einen echten Clipping-Bug:** bei 2 von 7 sichtbaren Zeilen wurde der Summary-Text ohne Ellipse abgeschnitten und der Stern-Button war komplett unsichtbar — die betroffenen Zeilen waren dabei erkennbar *kürzer* als die korrekten (Vermessung per Screenshot-Pixel-Differenz: alternierend ~82pt vs. ~103pt Zeilenabstand). Per gezieltem Root-Cause-Test (App komplett neu gestartet, OHNE die Einstellungen währenddessen live zu ändern) verschwand der Fehler vollständig — alle Zeilen korrekt. Mehrere anschließende Versuche, den Fehler durch gezielte Laufzeit-Einstellungsänderungen (Zeilen-Stepper hoch/runter, Textgrößen-Wechsel, sogar die exakte ursprüngliche Mehrfach-Änderungs-Sequenz in derselben Reihenfolge) erneut auszulösen, schlugen fehl — der Fehler trat nur einmalig auf, während im Hintergrund noch ein Feed-Refresh lief (sichtbar an „Aktualisiere 0/65" im Screenshot), der aktiv neue Zeilen in dieselbe Liste einfügte. **Arbeitshypothese (nicht abschließend verifiziert):** eine Race Condition zwischen SwiftUIs `List`-Zeilen-Materialisierung/-Wiederverwendung (macOS `List` ist NSTableView-backed) und einer gleichzeitigen `interfaceTextSize`/`imagePosition`/`summaryLineCount`-Änderung — bereits gerenderte Zeilen behalten dabei vermutlich kurzzeitig eine veraltete, aus den alten Einstellungen berechnete `rowHeight`, bis sie durch Scrollen oder Neu-Erzeugung aktualisiert werden; `ArticleRowHeightMetrics.height(...)` selbst bleibt dabei vermutlich korrekt (rein einstellungsbasiert, kein Content-Bezug), das Problem liegt eher in SwiftUIs Row-Reuse/Re-Layout-Timing als in der Formel. **Nicht behoben** — Reproduktionsversuche zu unzuverlässig für einen gezielten Fix-Versuch in dieser Session; falls das Verhalten erneut beobachtet wird (insbesondere: Einstellungen ändern, während im Hintergrund noch ein Feed-Refresh läuft), lohnt sich ein gezielter Reproduktionsversuch mit genau dieser Timing-Kombination als Ausgangspunkt.
...
6. **`ContentView.reloadFeedSnapshots()` auf `readAsync` umstellen** — einer der wenigen verbliebenen synchronen MainActor-DB-Reads im Hot-Path. **Umgesetzt (2026-07-28):** neue `FeedStore.sidebarFeedsAsync()` (nutzt GRDBs `readAsync`), `ContentView.reloadFeedSnapshots()` ruft sie jetzt statt der synchronen `sidebarFeeds()` auf.
   **Live-Verifikation (2026-07-28) via computer-use, gezielt auf das größte Korrektheitsrisiko getestet.** `.onChange(of: sqliteStatusVersion) { Task { await reloadFeedSnapshots() } }` startet bei jedem Bump einen neuen, nicht sequenzierten/nicht abgebrochenen async Task — bei schnell aufeinanderfolgenden Bumps könnte ein älterer, langsamerer Task theoretisch nach einem neueren fertig werden und dessen aktuelle Daten mit veralteten überschreiben (klassische Async-Race). Stresstest: 5 Artikel in sehr schneller Folge als gelesen markiert (`computer_batch`, alle 5 Klicks in einem Tool-Call ohne Zwischen-Screenshots) — dabei lief zufällig auch noch ein aktiver Hintergrund-Feed-Refresh mit, der selbst laufend neue Artikel einfügte und eigene Status-Version-Bumps auslöste (unfreiwillig, aber ein noch härterer Stresstest als geplant). Nach Abschluss (3s Wartezeit zum Beruhigen) waren alle vier global sichtbaren Zähler (`Alle Artikel`, `Ungelesen`, `Heute`, `Diese Woche`) übereinstimmend bei „9" — keine Divergenz zwischen ihnen, die auf eine veraltete Überschreibung hingedeutet hätte. Zusätzlich direkt gegengeprüft: die „Ungelesen"-Liste selbst zeigte exakt 9 Artikel, und die sichtbaren Pro-Feed-Badges in der Sidebar summierten sich korrekt darauf. Keine Stale-Data-Regression gefunden — die fehlende Sequenzierung/Cancellation ist zwar architektonisch nicht wasserdicht (ein theoretisches Race bleibt denkbar bei noch extremerer Last), aber im tatsächlichen Stresstest inkl. echtem, unfreiwillig gleichzeitigem Hintergrund-Refresh nicht reproduzierbar.
