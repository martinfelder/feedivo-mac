...
- Die kompakte Artikelsuche sitzt in der oberen macOS-Toolbar. Sie bleibt
  sichtbar, wenn eine Feed-/Tag-/Smartfolder-Liste aktiv ist, auch ohne
  Artikelauswahl, und filtert diese Liste über den SQLite/FTS-Pfad. Die frühere
  große Suchmaske wurde aus der Artikelliste entfernt, damit die mittlere Spalte
  leicht bleibt.
...
- **Stand 2026-07-02:** Der normale Feed-Lese-Pfad im Hauptfenster ist eine
  ...
  Feed-Snapshots erzeugt. Regeln für `hideArticle`, `notify` und `assignTag`
...
- Aktuell M4: Performance-Architektur vor Release. Die iCloud Sync Beta ist
  ...
  Feed-Titel und dem Snapshot-Wert `isNotificationEnabled` gemeldet. Neue
...
- 2026-07-04: Phasen 2, 3, 4, 7 und 9 wurden erfolgreich abgeschlossen.
  ...
  `ArticleListSnapshot`s. Die kompakte Suche in der oberen Toolbar nutzt den
...
- 2026-07-05: Kompakte Artikelsuche in die obere macOS-Toolbar verschoben. Das
  Suchfeld bleibt dadurch auch ohne Artikelauswahl sichtbar; die Artikelliste
  selbst bleibt frei von Such-Chroming und erhält den Suchtext als Binding aus
  `ContentView`.
...
- 2026-07-02: SQLite-first Sammelrefresh wendet erste Regeln ohne SwiftData-
  ...
  `ArticleRuleSnapshot`s; `SQLiteFeedRefreshService` prüft neue Artikel gegen
  diese Snapshots, setzt `hideArticle` direkt in `article_statuses` und gibt
  `notify`-Treffer als `RuleNotificationResult` zurück. `assignTag` wurde im
  Folgeslice über die SQLite-Tag-Tabellen ergänzt.
