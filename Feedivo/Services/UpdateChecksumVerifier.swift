...
/// `nonisolated`, damit Aufrufe von der MainActor-isolierten `UpdateInstaller`-Klasse aus
/// nicht implizit MainActor-isoliert werden (Whole-Branch-Review-Fund).
enum UpdateChecksumVerifier {
    ...
    nonisolated static func matches(computedHex: String, expectedHex: String) -> Bool {
        normalized(computedHex) == normalized(expectedHex)
    }

    nonisolated private static func normalized(_ hex: String) -> String {
